"""Azure Blob Storage static site provider."""
...
class AzureStaticProvider(StaticProvider):
    """Azure Blob Storage $web static site implementation."""
    ...
    def status(self, ctx: DeployContext) -> None:
        ...
        try:
            url = az(
                f"storage account show --name {storage_account} "
            ...
            console.print(
                f"  [bold]{ctx.name}[/bold] (static → {storage_account}): "
                f"{url or '[dim]no URL[/dim]'}"
            )
        except Exception:
            console.print(
                f"  [bold]{ctx.name}[/bold] (static → {storage_account}): "
                f"[dim]check manually[/dim]"
            )
